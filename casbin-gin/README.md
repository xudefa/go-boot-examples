...
演示 Casbin 权限管理器与 Gin HTTP 框架的集成，实现基于角色的访问控制（RBAC）。

## 功能特性

- **RBAC 权限模型**：基于角色的访问控制
- **Casbin 授权中间件**：在 Gin 路由上自动进行权限检查
...
curl -X POST -H 'X-User: bob' http://localhost:8090/api/data1/write
```
...
- `github.com/xudefa/go-boot/gin` — Gin 服务器集成
